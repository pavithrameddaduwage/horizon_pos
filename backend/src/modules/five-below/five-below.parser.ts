...
export interface FiveBelowParsedRow {
  reportFamily: string;
  reportDate?: string;
  department?: string;
...
export interface FiveBelowParseResult {
  success: boolean;
  retailer: 'FIVE_BELOW';
  detectedFamily: FiveBelowFamily;
  detectedDepartments: string[];
  totalRows: number;
  validRows: number;
  errorRows: number;
  data: FiveBelowParsedRow[];
...
export function parseFiveBelowCSV(
...
): FiveBelowParseResult {
  const result: FiveBelowParseResult = {
    success: true,
    retailer: 'FIVE_BELOW',
  ...
  for (let i = 1; i < parsed.data.length; i++) {
    ...
    const parsedRow: FiveBelowParsedRow = {
