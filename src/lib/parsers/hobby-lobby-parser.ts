// Dynamic Parser for Hobby Lobby POS CSVs & Department Reports
// Handles flexible headers (e.g., 'Vendor Numb', 'Buyer Numb', 'Item Descrip', '12 Month Sal'),
// extracts Department/Buyer dimensions, and parses filename metadata (Month, Year, Vendor #).
...
export function parseHobbyLobbyCSV(
  ...
  const effectiveMonthName = meta.monthName || MONTH_NAMES[effectiveMonth - 1] ? MONTH_NAMES[effectiveMonth - 1].charAt(0).toUpperCase() + MONTH_NAMES[effectiveMonth - 1].slice(1) : undefined;
  ...
  // Helper to find a field by fuzzy header matching
  ...
  parsed.data.forEach((row, idx) => {
    ...
    // Dynamic Department Construction from Buyer Number & Buyer Name
    ...
    // Monthly Sales Extraction (LY & CY)
    ...
    const parsedRow: HobbyLobbyRow = {
      ...
      reportingMonthName: effectiveMonthName,
    };
